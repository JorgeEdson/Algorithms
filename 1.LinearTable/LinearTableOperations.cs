...
namespace _1.LinearTable
{
    public static class LinearTableOperations
    {
        ...
        public static void ShellSorting(int[] linearTableInput)
        {
            Console.WriteLine("\nShell Sorting Linear Table\n");
            for (int gap = linearTableInput.Length / 2; gap > 0; gap = gap / 2)
            {
                for (int i = gap; i < linearTableInput.Length; i++)
                {
                    int j = i;
                    while (j - gap >= 0 && linearTableInput[j] < linearTableInput[j - gap])
                    {
                        Swap(linearTableInput, j, j - gap);
                        j -= gap;
                    }
                }
            }
